...
auto AnalyticalSolver::calculate_horizontal_fall_distance_m(double fall_time) const -> double
{
  ...
  const double a3 =
    input.drag * (kGravity * input.lift * input.mass 
    - input.drag * (lift_squared - 1.0) * input.attack_speed) / (6.0 * mass_squared);
  ...
  const double a5 = drag_cubed * (kGravity * input.lift * lift_squared * input.mass 
    - input.drag * lift_squared * one_plus_lift_squared * input.attack_speed)
    / (12.0 * one_plus_lift_squared * mass_fourth);

  double horizontal_fall_distance_m = fall_time * (input.attack_speed 
    + fall_time * (a2 + fall_time * (a3 + fall_time * (a4 + fall_time * a5))));
...
auto AnalyticalSolver::solve(const pointmath::Point& drone_position,
                             const pointmath::Point& target_position)  -> dto::DropSolution 
{
    validate_input();
