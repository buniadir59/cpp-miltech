...
// завантаження конфігу, створення компонентів через фабрику
	...
	std::thread providerThread(&ThreadSafeTargetProvider::run, ...);
	std::thread physicsThread(&DronePhysics::run, ...);
	std::thread missionThread(&MissionProcessor::run, ...);

	while (!provider->isThreadReady() || !missionThread->isThreadReady()) || !physicsThread->isThreadReady())
...
auto main() -> int
{
  ...
  try {
    ...
    auto simClock = std::make_unique<ManualSimulationClock>();  //TODO remove
